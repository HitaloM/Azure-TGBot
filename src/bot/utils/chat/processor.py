...
def clean_and_format_llm_text(text: str) -> str:
    """
    Clean LLM generated text by removing excessive newlines and markdown dividers.
    Preserves code blocks formatting.

    Args:
        text: Raw text generated by LLM

    Returns:
        Cleaned text with consistent formatting
    """
    # Split text to preserve code blocks
    parts = []
    is_code_block = False
    code_block_buffer = []
    current_buffer = []
    ...
    for line in text.split("\n"):
        ...
        if line.strip().startswith("```"):
            if not is_code_block:
                # Start of code block - flush current buffer
                if current_buffer:
                    parts.append("\n".join(current_buffer))
                    current_buffer = []
                ...
                # End of code block - flush code buffer
                code_block_buffer.append(line)
    ...
    if current_buffer:
        parts.append("\n".join(current_buffer))
    if code_block_buffer:  # Handle unclosed code blocks
        parts.append("\n".join(code_block_buffer))
    ...
    for i, part in enumerate(parts):
        if not part.strip().startswith("```"):
            ...
            # Remove excessive newlines (more than 2) but keep normal paragraph breaks
